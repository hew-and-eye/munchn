<template>
<div class="calendar-container" :style="scaledCalendarStyle" ref="calendarRef">
  <!-- Calendar -->
...
export default {
  ...
  computed: {
    scaledCalendarStyle () {
      const maxDrawerWidth = Math.min(window.innerWidth - 125, 240)
      const scaleVal = maxDrawerWidth < 321 ? maxDrawerWidth / 240 : 1
      return {
        transform: `scale(${scaleVal})`,
        margin: -(1 - scaleVal) * 100 + '%',
        'margin-top': -(1 - scaleVal) * 50 + '%',
        'margin-bottom': -(1 - scaleVal) * 50 + '%'
      }
    },
    /**
